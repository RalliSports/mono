...
describe("RalliBet Comprehensive Tests", () => {
  ...
  let lineId7: BN = new BN(2020);
  let lineId8: BN = new BN(3030);
  let lineId9: BN = new BN(4040);
  let lineId10: BN = new BN(5050);
  let lineId11: BN = new BN(6060);
  let lineId12: BN = new BN(7070);
  let numberOfLines = 3;
  ...
  let predictedValue7 = 5.5;
  let predictedValue8 = 75.5;
  let predictedValue9 = 33.5;
  let predictedValue10 = 44.5;
  let predictedValue11 = 88.5;
  let predictedValue12 = 12.5;
  ...
  let newPredictedValue7 = 6.5;
  let newPredictedValue8 = 80.5;
  let newPredictedValue9 = 35.5;
  let newPredictedValue10 = 45.5;
  let newPredictedValue11 = 90.5;
  let newPredictedValue12 = 15.5;
  ...
  let athleteId7: BN = new BN(10007);
  let athleteId8: BN = new BN(10008);
  let athleteId9: BN = new BN(10009);
  let athleteId10: BN = new BN(10010);
  let athleteId11: BN = new BN(10011);
  let athleteId12: BN = new BN(10012);
  ...
  describe("Resolve Game Batch Tests", () => {
    ...
    let newLineId1 = new BN(12000);
    let newLineId2 = new BN(12001);
    let newLineId3 = new BN(12002);
    ...
    before(async () => {
      ...
      // Single user game (edge case)
      const newGameId14 = new BN(12014);
      const [newGame14] = PublicKey.findProgramAddressSync(
        [Buffer.from("game"), newGameId14.toArrayLike(Buffer, "le", 8)],
        program.programId
      );
    ...
    it("should successfully resolve a 2-player game where one player wins", async () => {
      ...
      await program.methods
        .resolveGameBatch(entryFeePercentage, 0)
        .accounts(accounts)
        .remainingAccounts([
          {
            pubkey: user1TokenAccount,
    ...
    it("should allow partial winner resolution in first batch", async () => {
      const accounts = {
        admin: adminKeypair.publicKey,
        game: newGame7PK,
        gameEscrow: newGameEscrow7PK,
      ...
      const gameData = await program.account.game.fetch(newGame7PK);
      expect(gameData.payoutProgress).to.equal(1); // Paid out 1 winner
      expect(gameData.status).to.deep.equal({ resolving: {} });
    });

    it("should successfully resolve a 7-user game where 2 win with low fees", async () => {
      // Game already paid 1 winner (user1), now we need to pay the remaining winner (user2)
      const user2BalanceBefore = await connection.getTokenAccountBalance(
        user2TokenAccount
      );
      const treasuryBalanceBefore = await connection.getTokenAccountBalance(
      ...
      // Only provide the REMAINING winner (user2), since user1 was already paid in previous test
      await program.methods
        .resolveGameBatch(lowFeePercentage, 1) // batch_index 1 since we're continuing
        .accounts(accounts)
        .remainingAccounts([
          { pubkey: user2TokenAccount, isWritable: true, isSigner: false }, // Only user2
        ])
        .signers([adminKeypair])
        .rpc();
      ...
      // Check that user2 (the remaining winner) received the correct payout
      const user2BalanceAfter = await connection.getTokenAccountBalance(
        user2TokenAccount
      );
      const user2Change = new anchor.BN(user2BalanceAfter.value.amount).sub(
      ...
      // Verify game is fully resolved
      const gameAfter = await program.account.game.fetch(newGame7PK);
      expect(gameAfter.status).to.deep.equal({ resolved: {} });
      expect(gameAfter.payoutProgress).to.equal(2);
    });
    ...
    it("should successfully resolve a 5-user game where 4 win with medium fees", async () => {
      ...
      await program.methods
        .resolveGameBatch(mediumFeePercentage, 0)
        .accounts(accounts)
        .remainingAccounts([
          { pubkey: user1TokenAccount, isWritable: true, isSigner: false },
          { pubkey: user2TokenAccount, isWritable: true, isSigner: false },
