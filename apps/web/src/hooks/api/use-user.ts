...
import {
  UserServiceFindOne,
  UpdateUserDtoType,
  GamesServiceGetMyOpenGames,
  GamesServiceGetMyCompletedGames,
...
export function useUser() {
  ...
  const currentUserQuery = useQuery({
    ...
    queryFn: () => api.get<UserServiceFindOne>('/api/read-current-user'),
    staleTime: 5 * 60 * 1000, // 5 minutes
    enabled: !!isConnected,
  })

  const updateMutation = useMutation({
    mutationFn: (data: UpdateUserDtoType) => api.post<UserServiceUpdateUser>('/api/update-user', data),
    onSuccess: () => {
  ...
  const myOpenGamesQuery = useQuery({
    ...
    queryFn: () => api.get<GamesServiceGetMyOpenGames>('/api/read-my-open-games'),
  ...
  const myCompletedGamesQuery = useQuery({
    ...
    queryFn: () => api.get<GamesServiceGetMyCompletedGames>('/api/read-my-completed-games'),
