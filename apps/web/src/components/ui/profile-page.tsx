          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div className="bg-gradient-to-br from-[#00CED1]/10 to-[#00CED1]/5 rounded-2xl p-4 border border-[#00CED1]/20">
              <div className="text-center">
                <div className="text-2xl font-bold text-[#00CED1] mb-1">${user.balance}</div>
        ...
        <div className="px-6 mb-6">
          ...
          <div className="space-y-3">
            {recentActivity.map((activity) => (
              <div key={activity.id} className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-1">
                      <span
                        className={`w-3 h-3 rounded-full ${activity.type === 'win' ? 'bg-emerald-400' : 'bg-red-400'}`}
                      ></span>
                      <span className="text-white font-semibold text-sm">{activity.game}</span>
                    </div>
                    <div className="text-slate-400 text-xs mb-2">{activity.date}</div>
                  </div>
                  <div className={`font-bold text-sm ${activity.type === 'win' ? 'text-emerald-400' : 'text-red-400'}`}>
                    {activity.type === 'win' ? '+' : '-'}${activity.amount}
                  </div>
                </div>
                <div className="flex flex-wrap gap-1">
                  {activity.players.map((player, index) => (
                    <span key={index} className="bg-slate-700/50 px-2 py-1 rounded-md text-xs text-slate-300">
                      {player}
                    </span>
                  ))}
                </div>
        ...
        <div className="px-6 pb-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            {achievements.map((achievement, index) => (
              ...
              >
                <div className="text-center">
                  <div className={`text-lg mb-1 ${achievement.unlocked ? 'text-[#FFAB91]' : 'text-slate-500'}`}>
                    {achievement.unlocked ? '🏆' : '🔒'}
                  </div>
                  <div
                    className={`font-semibold text-xs mb-1 ${achievement.unlocked ? 'text-white' : 'text-slate-500'}`}
                  >
                    {achievement.name}
                  </div>
                  <div className="text-slate-400 text-xs leading-tight">{achievement.description}</div>
                </div>
              </div>
            ))}
          </div>
