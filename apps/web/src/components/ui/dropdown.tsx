    ...
    <div ref={dropdownRef} className={`relative ${className}`}>
      ...
      <button
        ...
        className={`w-full px-4 py-3 bg-slate-800/50 border border-slate-700/50 rounded-xl text-left focus:ring-2 focus:ring-[#00CED1] focus:border-[#00CED1] transition-all duration-200 ${
          disabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-800/70 cursor-pointer'
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            {selectedOption ? (
              <>
                {showIcons && selectedOption.icon && <span className="text-lg">{selectedOption.icon}</span>}
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-1 bg-slate-800/95 backdrop-blur-md border border-slate-700/50 rounded-xl shadow-2xl max-h-60 overflow-hidden">
          {searchable && <SearchBar searchTerm={searchTerm} setSearchTerm={setSearchTerm} />}
          ...
          <div className="max-h-48 overflow-y-auto">
            {filteredOptions.length > 0 ? (
              filteredOptions.map((option) => (
                ...
                >
                  <div className="flex items-center space-x-2">
                    ...
                    <span className={`${option.value === value ? 'text-[#00CED1]' : 'text-white'}`}>
                      {option.label}
                    </span>
                  </div>
                </button>
              ...
              <div className="px-4 py-3 text-slate-400 text-center">No options found</div>
            )}
          </div>
        </div>
      )}
